...
exports.resetUserPassword_v1 = (req, res) => {
    validateResetPasswordRequest(req)
    ...
    .catch((errorCode) => {
        switch(errorCode) {
            ...
            case 502: return sendError(res, errorCode, 'Failed to reset user\'s password!');
            default: return sendError(res, 500, 'Something went wrong!');
        }
    })
}

function validateResetPasswordRequest(request) {
    ...
    return new Promise(function(resolve, reject){
        ...
        .then( 
            (response) => {
                ...
                resolve("Successfully reset user's password!");
            }
        )
        .catch((error) => { 
            console.log(error);
